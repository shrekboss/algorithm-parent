package org.crayzer.leetcode.editor.en.sort.heap;
...
public class MyHeapNode {
  ...
  private static void heapify(CodeNode[] a, int n, int i) {
    while (true) {
      int maxPos = i;
      if (i * 2 <= n && a[i].frequence < a[i * 2].frequence) maxPos = i * 2;
      if (i * 2 + 1 <= n && a[maxPos].frequence < a[i * 2 + 1].frequence) maxPos = i * 2 + 1;
      if (maxPos == i) break;
      swap(a, i, maxPos);
      i = maxPos;
    }
